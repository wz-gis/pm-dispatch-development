...
- 合并分发：把同工程、同归属、同回归面的相似 Bug / Spec 合并到一个批处理 Worker，减少线程和巡检开销。
...
```text
合并分发 BUG-001、BUG-002、BUG-003 到同一个 Sesame 线程，但保留独立证据和状态。
```
...
为了避免“局部通过、真实场景回退”，每次实现或联调收口都必须写清回归矩阵：

```text
改动面：
用户原始路径：
启动形态：dev / jar / release / mock / real
测试数据：projectId / taskId / snapshotId / 文件 / 日志
L1：
L2：
L3：
L4：
...
- UI 改动必须有 Browser 点击证据，不能只用 API 或 SQL 代替。
- SQL、表结构、启动脚本、打包脚本和 release 静态资源改动必须验证升级或 release 路径。
- 涉及扫描、索引、日志诊断、规则、报告时，必须验证既有工程和存量数据不回退。
...
每个任务先写入 `task.yaml.dispatch.strategy`，用最轻可验证方式推进：

| Strategy | 用法 |
| --- | --- |
| `direct` | 小修、文案、配置、单文件低风险任务；当前线程直接处理，不创建 worker/run/heartbeat。 |
| `single-worker` | 单工程但需要实现 + 验证；一个 worker 完成，不拆实现和联调。 |
| `batch-worker` | 2-4 个相似任务共享一个 worker，但 evidence 和结论独立。 |
| `full-dispatch` | 跨工程、DB/release、真实环境或高风险任务，才走完整 PM gate。 |

默认不要从 full-dispatch 开始。direct 过程中发现证据不足、owner 不清或需要 Browser/API/SQL/release 链路时，再升级策略。
