...
3. **定义任务**
   ...
   - 用可观察证据写验收标准，不写模糊描述。
   - 记录 lifecycle、verification、blockers、closure、dependencies、resources、runs。
...
4. **分发**
   ...
   - 自动巡检 prompt 必须读取看板、任务元数据、证据、决策和 worker 线程；worker 仍在运行时只续租 lease 并汇报状态，不做文档 churn；worker 完成后回收证据、更新 gate 并提交 PM 文档。
...
5. **巡检**
   ...
   - worker 仍在运行且 lease 未过期时，只简短汇报状态并续租，不做文档 churn。
